...
class Nilai extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('nilai/tabel', $data);
    }
    ...
    public function tambah()
    {
        ...
        $this->form_validation->set_rules('nama_nilai', 'Nama Nilai', 'required|is_unique[nilai.nama_nilai]');
        ...
        if ($this->form_validation->run()) {
            $params = array(
                ...
                'nama_nilai' => $this->input->post('nama_nilai', TRUE),
            );
    ...
    public function ubah($id_nilai = '')
    {
        ...
        if (empty($data['nilai'])) {
            ...
            $this->form_validation->set_rules('batas_1', 'Nilai Batas 1', 'required|numeric');
            $this->form_validation->set_rules('batas_2', 'Nilai Batas 2', 'required|numeric');
            $this->form_validation->set_rules('nama_nilai', 'Nama Nilai', 'required|callback_cek_unik_nama_nilai');
    ...
    public function cek_unik_nama_nilai($nama_nilai)
    {
        $query = $this->nilai_model->cek_unik_nama_nilai($nama_nilai, $this->input->post('nama_nilai_awal'));
        if ($query->num_rows() > 0) {
            $this->form_validation->set_message('cek_unik_nama_nilai', '{field} sudah ada');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function prioritas()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            $this->session->set_flashdata('pesan_sukses', '<div class="alert alert-success" role="alert">Nilai perbandingan berhasil disimpan</div>');
        }
        ...
        if (isset($_POST['check'])) {
            ...
            $prioritas_sub = $this->ahp_get_prioritas_nilai($prioritas);
    ...
    function ahp_get_prioritas_nilai($prioritas)
    {
        $prioritas_sub = array();
    ...
    public function tampil_data_1($matrik_nilai, $jumlah_kolom)
    {
        ...
        $list_data = '';
        $list_data .= '<tr><td class="text-center">Nilai</td>';
        foreach ($nilai as $row) {
            $list_data .= '<td class="text-center">' . $row->nama_nilai . '</td>';
        }
        $list_data .= '</tr>';
        $i = 0;
        foreach ($nilai as $row) {
            $list_data .= '<tr>';
            $list_data .= '<td style="text-align: left">' . $row->nama_nilai . '</td>';
            $ii = 0;
            foreach ($nilai as $row2) {
    ...
    public function tampil_data_2($matrik_normalisasi, $prioritas, $prioritas_sub)
    {
        ...
        $list_data2 = '';
        $list_data2 .= '<tr><td class="text-center">Nilai</td>';
        foreach ($nilai as $row) {
            $list_data2 .= '<td class="text-center">' . $row->nama_nilai . '</td>';
        }
        ...
        foreach ($nilai as $row) {
            $list_data2 .= '<tr>';
            $list_data2 .= '<td style="text-align: left">' . $row->nama_nilai . '</td>';
            $jumlah = 0;
            $ii = 0;
            foreach ($nilai as $row2) {
    ...
    public function tampil_data_3($matrik_baris, $jumlah_matrik_baris)
    {
        ...
        $list_data3 = '';
        $list_data3 .= '<tr><td class="text-center">Nilai</td>';
        foreach ($nilai as $row) {
            $list_data3 .= '<td class="text-center">' . $row->nama_nilai . '</td>';
        }
        ...
        foreach ($nilai as $row) {
            $list_data3 .= '<tr>';
            $list_data3 .= '<td style="text-align: left">' . $row->nama_nilai . '</td>';
            $ii = 0;
            foreach ($nilai as $row2) {
    ...
    public function tampil_data_4($jumlah_matrik_baris, $prioritas, $hasil_tabel_konsistensi)
    {
        ...
        $list_data4 = '';
        $list_data4 .= '<tr><td class="text-center">Kriteria</td>';
        ...
        foreach ($nilai as $row) {
            $list_data4 .= '<tr>';
            $list_data4 .= '<td style="text-align: left">' . $row->nama_nilai . '</td>';
